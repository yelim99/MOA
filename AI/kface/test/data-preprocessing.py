    ...
    for j, c in enumerate(class_names):
        ...
        for i, (img, txt) in enumerate(zip(imgs, txts)):
            ...
            with open(txt, 'r') as f:
                ...
                # 클래스 인덱스 J가 390 이상이면 검증용 데이터로 저장
                if j >= 390:
...
# 학습, 검증용 데이터셋 만들고 numpy 배열로 저장
# references.csv 파일을 생성하고, 각 쌍의 이미지와 매칭 여부를 기록
def make_pair_numpy():
    ref_imgs = []
    query_imgs = []
    is_same = []
    ...
    with open("../data/references.csv", 'w', encoding='utf-8') as f:
        ...
        # 검증용 데이터에서 임의의 레퍼런스 이미지를 선택하고, 
        # 112x112 크기로 조정하여 numpy 배열로 저장
        for _ in range(400):
            ...
            # 쿼리 이미지와 레퍼런스 이미지가 같은 클래스에 속할 확률을 50%로 설정하고, 
            # 일치 여부를 is_same 리스트에 저장
    ...
    # 각 리스트를 numpy 파일로 저장하여 나중에 학습에 사용
